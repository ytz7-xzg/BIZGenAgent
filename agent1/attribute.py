...
# -*- coding: utf-8 -*-
"""
Boogu-Turbo attribute 维度修复流水线（迭代版，改编自 bizgen_attribute_fix_8_10.py）
...
流程：
mark/ 评测 json → 找 attribute 维度 result=false 的题
→ Gemini 看当前图，返回按优先级排序的 attribute 编辑计划
→ 只执行第 1 个编辑
→ 用编辑后的图重新请求 Gemini
→ 最多重复 MAX_ATTRIBUTE_ROUNDS 轮
→ 结果保存到 8-19edit/（文件名含 domain_attribute_id，与其他维度不冲突）

用法（bizgeneval 环境，2 号卡）：
    conda activate bizgeneval
    CUDA_VISIBLE_DEVICES=2 python boogu_attribute_fix.py
"""
...
SEED = 42
SKIP_EXISTING = True
TARGET_DIMENSION = "attribute"
CLEAN_CROP_PROMPT_V3 = True
MAX_ATTRIBUTE_ROUNDS = 25
...
def build_instruction(e):
    ...
    if desired:
        return f"Change only the {target} to {desired}."
    return ""
...
def normalize_edit(e):
    ...
    return {
        "attribute_type": clean_field(e.get("attribute_type"), "other"),
        ...
        "preserve": clean_field(
            e.get("preserve"),
            "every target property except the single property named in the action, and all surrounding elements",
        ),
        "current_state": clean_field(e.get("current_state"), "current incorrect state"),
        "desired_state": clean_field(e.get("desired_state"), "required corrected state"),
        "bbox": bb,
...
Write one complete natural-language image-editing instruction for the visible target in this crop whose current state is:
{edit['current_state']}
...
- Describe the target with enough crop-local visible cues to distinguish it from similar elements.
- State exactly one editing action.
- Explicitly preserve: {edit['preserve']}
- Explicitly name the actually visible nearby text labels, icons, borders, accents, and layout edges that must remain unchanged.
- Treat all non-target crop content as fixed pixels: do not redraw, rewrite, move, blur, resize, duplicate, erase, or restyle it.
...
def gemini_plan(image_source, prompt, failed_questions, applied_history=None, blocked_checks=None, rejection_history=None, retries=3):
    """Gemini 看当前图 + attribute 失败问题 + 历史修改 → 返回按优先级排序的计划。"""
    ...
    if applied_history:
        history_text = "\n".join(f"- {x}" for x in applied_history)
    else:
        history_text = "No edits have been applied yet."

    planner_prompt = f"""This image was generated from the prompt:
\"\"\"{prompt}\"\"\"

The following edits have ALREADY been applied in previous rounds:
{history_text}
...
Discover visible attribute discrepancies yourself; do not assume an evaluator has identified them for you.
Plan minimal local edits only for discrepancies visibly supported by the current image and original prompt.
Possible discrepancies may concern color, shape, style, icon, object identity, material, or quantity.
Do not repeat an already applied edit unless it is still clearly wrong.

Return STRICT JSON only: no markdown, no explanation, no extra text.
Use EXACTLY this list format:
[
  {{
    "attribute_type": "color | shape | style | icon | object | quantity | material | other",
    "target_object": "the exact visual element to edit",
    "failed_check": "a concise description of the self-discovered visible discrepancy",
    "visual_signature": "current visible type, color/shape, and unique attached text or local neighbor cues",
    "reference_context": "visible scale, gridline, axis tick, or nearby guide needed for this action; empty if unnecessary",
    "preserve": "target properties and nearby content that must remain unchanged",
    "current_state": "the incorrect state currently visible in the image",
    "desired_state": "the exact corrected state supported by the original prompt",
    "bbox": [x1, y1, x2, y2],
    "instruction": "one precise, self-contained editing command"
  }}
]
...
- If no visible attribute discrepancy remains in the CURRENT image, return an empty list: [].
- bbox MUST use normalized [0,1] xyxy corner coordinates, not xywh and not 0~1000.
- Use the smallest bbox that still contains the complete target and every local cue or reference needed for this edit.
- target_object must identify the exact element, not just say "object" or "area".
- current_state and desired_state must be concrete and visually checkable.
- instruction must include target_object and desired_state, and must start with "Change only", "Replace only", "Add", or "Remove".
- For color: include the exact color name and a hex color if it can be inferred.
- For quantity: state the exact final count and exactly what should be added or removed.
- For icon/object: state both the current icon/object and the exact replacement.
- For shape/style/material: state the exact required shape/style/material.
- One atomic action per round. Never merge color, height, size, text, or position changes.
- Attribute corrections only; preserve all existing text, layout, positions, and unrelated visual elements.
- Do not redesign the whole image."""
    ...
    for attempt in range(retries):
        try:
            ...
            # Gemini 明确判断当前图没有剩余问题：正常停止，不算失败
            if len(payload) == 0:
                return [], {
...
def save_plan_json(fname, it, img_path, save_path, failed, plan_info, round_idx, selected_edit, planned_count, applied_history):
...
def main():
    ...
    for i, (it, fname, img_path, failed) in enumerate(todo):
        ...
        try:
            for round_idx in range(1, MAX_ATTRIBUTE_ROUNDS + 1):
                print(f"  [ROUND {round_idx}/{MAX_ATTRIBUTE_ROUNDS}] Gemini 基于当前图重新规划")
                ...
                plan_path = save_plan_json(
                    ...
                    plan_info, round_idx, selected_edit,
                    len(edits) if edits else 0,
                ...
                if edits is None:
                    ...
                    break
                if len(edits) == 0:
                    print("  [STOP] Gemini 判断当前图没有剩余 attribute 问题")
                    break
                ...
                print(f"  [EDIT] bbox={e['bbox']} crop_box={box}")
                fixed = edit_region(
                    pipe,
                    region,
                    e["editor_prompt"],
                ...
                applied_history.append(
                    f"Round {round_idx}: {e['instruction']} "
                    f"(target={e['target_object']}, desired={e['desired_state']})"
                )
            ...
            if applied_rounds > 0:
                ...
                print("  [FAIL] 没有实际应用任何 attribute 编辑，未保存结果")
                n_fail += 1
