...
def compose_qwen_prompt(e):
    ...
    return (
        f"{operation} only the target so that {e['desired_state']}. "
...
def edit_region(pipe, region_img, qwen_prompt, seed, marked_img=None):
    ...
    inputs = {
        ...
        "generator": generator,
        "true_cfg_scale": 4.0,
        "negative_prompt": "unrelated changes, duplicated elements, missing content, distortion, broken borders, visible seams, redesigned image",
    ...
    for i, (it, fname, img_path, failed) in enumerate(todo):
        ...
        try:
            for round_idx in range(1, MAX_LAYOUT_ROUNDS + 1):
                ...
                marker_bbox = e.get("bbox") or e.get("source_bbox") or e["edit_bbox"]
                before = img.copy()
                round_dir, marked_region = prepare_round_artifacts(
                    INTERMEDIATE_DIR, fname, round_idx, before, region, box, marker_bbox
                )
