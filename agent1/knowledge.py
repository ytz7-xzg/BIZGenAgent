...
- Describe the target precisely enough to distinguish it from every similar visible element in the crop.
...
def paste_difference_mask(bg, original_crop, edited_crop, crop_box, bbox,
    ...
    background = ~allowed
    shift = np.median(original[background] - edited[background], axis=0) if background.any() else np.zeros(3)
    corrected = np.clip(edited + np.asarray(shift).reshape(1, 1, 3), 0, 255)
    ...
    base[cy1:cy2, cx1:cx2][mask] = corrected.astype(np.uint8)[mask]
    return Image.fromarray(base, mode="RGB"), Image.fromarray((mask.astype(np.uint8) * 255), mode="L")
