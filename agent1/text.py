...
# -*- coding: utf-8 -*-
"""
Boogu-Turbo text 维度修复流水线（knowledge 版的结构化方法）
流程：mark/ 评测 json → 找 text 维度 result=false 的题
     → Gemini 看图定位错误文字 + 给出结构化修正计划（现状/正确内容/理由）
     → Qwen-Image-Edit-2511 裁区域局部修图 → 羽化贴回
     → 存 8-19edit/（文件名含 domain_text_id，与 knowledge 修复不冲突）
     → 每张图的计划存 gemini_text_plans/ 便于追溯

用法（bizgeneval 环境，2 号卡）：
    conda activate bizgeneval
    CUDA_VISIBLE_DEVICES=2 python boogu_text_fix.py
"""
...
def build_instruction(e):
    ...
    if instruction:
        return instruction
    target = clean_field(e.get("target_object"), "target text")
    correct = clean_field(e.get("correct_content"))
    if correct:
        return f"Replace only the {target} with the correct text: {correct}."
    return ""
...
def normalize_edit(e):
    ...
    if bb is None or not instruction:
        return None
    target_object = clean_field(e.get("target_object"), "target text")
    ...
    return {
        ...
        "preserve": clean_field(
            e.get("preserve"),
            "the original font style, size, color, alignment, position, and all surrounding content",
        ),
        "current_content": clean_field(e.get("current_content"), "current incorrect text"),
        "correct_content": clean_field(e.get("correct_content")),
        "correction_reason": clean_field(e.get("correction_reason"), "The visible text does not match the original prompt."),
        "bbox": bb,
...
def compose_qwen_prompt(e):
    ...
    return (
        f"Correct only the visible text '{e['current_content']}'. Its exact final text must be "
        f"'{e['correct_content']}'. Preserve its font, weight, color, size, orientation and position. "
...
[{{
  "target_object": "exact text element",
  "failed_check": "a concise description of the self-discovered visible discrepancy",
  "visual_signature": "current visible text plus its style and nearby local cues",
  "reference_context": "nearby heading, row, card, or label that disambiguates this text; empty if unnecessary",
  "preserve": "font, size, color, position, and surrounding content to keep unchanged",
  "current_content": "currently visible incorrect text",
  "correct_content": "exact required final text",
  "correction_reason": "why this fixes the failed check",
  "bbox": [x1, y1, x2, y2],
  "crop_rotation": "none | clockwise_90 | counterclockwise_90",
  "instruction": "one exact replacement command"
}}]
...
- crop_rotation is the rotation applied before editing to make text horizontal and left-to-right. Use clockwise_90 for bottom-to-top vertical text, counterclockwise_90 for top-to-bottom vertical text, and none for horizontal text.
...
Rules:
- bbox: normalized [0,1] coordinates; use the smallest box that still contains the complete text element and every local cue needed to identify it.
...
def edit_region(pipe, region_img, qwen_prompt, seed):
    ...
    inputs = {
        ...
        "generator": generator,
        "true_cfg_scale": 4.0,
        "negative_prompt": "wrong text, misspelled words, garbled characters, unrelated changes, duplicated elements, missing content, distortion, visible seams, redesigned image",
